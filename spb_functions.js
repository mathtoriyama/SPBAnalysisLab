...
function calculate_energy(eff_mass) {
	var eng = [];
	var kv = calculate_kvector();
	for (var x = 0; x < kv.length; x++) {
		eng.push((Math.pow(kv[x],2) * Math.pow(1.05457182E-34,2)/ (2 * eff_mass))/1.602E-19/9.109E-31)
...
function calculate_kvector() {
	var k_vector = [];
	for (var i = -1E10; i <= 1E10; i=i+1E7) {
		k_vector.push(i);
	}
	return k_vector;
}
